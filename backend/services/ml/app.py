...
# CORS: defaults to permissive for local dev; set ALLOWED_ORIGINS in production
...
class SentimentResponse(BaseModel):
    ...
    label: str = Field(
        ..., description="Classification label: positive, negative, or neutral"
    )
...
async def analyze_text_sentiment(request: SentimentRequest):
    ...
    try:
        ...
        logger.info(
            f"Analysis complete: {result['label']} (score: {result['sentiment']})"
        )
